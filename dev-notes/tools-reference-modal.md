...
The modal sorts tools by name and renders a compact table: tool name, origin, and description character count on one line. Origins are `Built in` or `Extension: <name>`; `CodingSession` exposes the extension runtime's registration metadata so overrides are also attributed correctly. Its focused search field filters names, labels, and descriptions case-insensitively as the user types. Up and Down move through results; Enter or a mouse click opens the selected tool's full description. Escape returns from details or closes the browser without changing the prompt. Dedicated messages cover sessions with no tools and searches with no matches.
