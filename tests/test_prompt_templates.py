...
@pytest.mark.parametrize("name", ["prompts", "tools"])
def test_reserved_picker_template_is_ignored_with_diagnostic(tmp_path: Path, name: str) -> None:
    prompts_dir = tmp_path / "prompts"
    prompts_dir.mkdir()
    reserved_path = prompts_dir / f"{name.upper()}.md"
    ...
    assert f"reserved by the built-in /{name} command" in diagnostics[0].message
